...
| `HTTP 400` + `PGRST204` + "Could not find the 'hero_id' / primary damage column" | Live `scores` table predates one leaderboard migration — column wasn't added | Run the **HOT FIX SQL** below in Supabase SQL editor |
...
### HOT FIX: missing leaderboard detail columns

If the failure banner reads `PGRST204 ... Could not find the 'hero_id' column` or one of the `primary_damage_*` columns, the live Supabase project is one migration behind. The client auto-strips missing optional columns and retries, so submits still succeed, but the omitted leaderboard detail will not appear globally until the migration is run.
...
4. Wait ~10 seconds for the PostgREST schema cache to refresh, then submit a new score. The failure banner should now show ✓ SCORE SUBMITTED and the hero suffix plus Top Damage column should appear.

After running this once, future submissions land cleanly with the `⚔ HeroName` suffix in the NAME column and the run's primary damage dealer in the TOP DMG column.
