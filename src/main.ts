...
async function boot() {
  ...
  function closeRampartPlacementTray(): void {
    ...
    state.hint = 'Stone Rampart placement cancelled — the rampart is still in your inventory.';
    document.getElementById('rampart-rotate-chip')?.remove();
    renderer.drawRampartPreview([], false);
  }
  function syncRampartRotateChip(): void {
    ...
    const traySig = `${collapsed ? '1' : '0'}|${current}`;
    if (chip.dataset.rampartTraySig === traySig) return;
    ...
    if (collapsed) {
      ...
      chip.querySelector<HTMLButtonElement>('#rampart-tray-close')?.addEventListener('click', ev => {
        ev.stopPropagation();
        ev.preventDefault();
        closeRampartPlacementTray();
      });
      return;
    }
  ...
  //
  // 2026-05-23 — Recipe-row clicks now open a centered modal (not an
  // inline drop-down), so no in-panel state to persist.
  function updateProspectSidebar() {
    ...
    if (combos.length > 0) {
      ...
      // 2026-05-23 — Click a recipe row to pop a centered modal with the
      // combo tower's full breakdown: sprite, damage type, DPS, range,
      // attack rate, crit profile, ability text, and the recipe
      // ingredients. Replaces the earlier inline drop-down attempt the
      // user flagged as not working / hard to notice. The modal is more
    ...
    // 2026-05-23 — Wire recipe-row clicks → centered combo info modal.
    // Each row is the click target; the modal pops with the full combo
    // breakdown (sprite, stats, ability, ingredients). No inline state
    // to persist anymore — the modal is its own ephemeral DOM that the
    // user dismisses with ✕ / Escape / backdrop click.
    panel.querySelectorAll('.recipe-row').forEach(row => {
      const comboKey = (row as HTMLElement).dataset.comboType ?? '';
      if (!comboKey) return;
      (row as HTMLElement).onclick = () => showComboInfoModal(comboKey);
    });
  }
  ...
  function showQuestsModal() {
    ...
    const grandBanner = `<div style="background:linear-gradient(180deg,#2a1f12,#1a1410);border:2px solid #d4af37;padding:9px 12px;margin-bottom:8px;text-align:center">
      ...
      <div style="font-size:10px;color:#cdb98a;margin-top:3px">Clear all ${QUESTS.length} quests to bank an additional <b style="color:#88ff88">+500g</b> capstone</div>
    ...
    const completedHtml = completed.length > 0
      ? `<div style="margin-top:10px;padding-top:10px;border-top:1px solid #5a4a30;font-size:11px;color:#aa9a4a"><b style="color:#88ff88">Completed:</b> ${completed.length} / ${QUESTS.length}</div>`
      : '';
    const modal = document.createElement('div');
  ...
  function tickQuests() {
    ...
    // rewards land, check whether a whole tier (or every quest) just closed
  ...
  function grantQuestTierBonus(tierKey: 'EARLY' | 'MID' | 'LATE' | 'ALL') {
    ...
    const questCount = tierKey === 'ALL'
      ? QUESTS.length
      : QUESTS.filter(q => q.tier === tierKey).length;
    const lines: Record<'EARLY' | 'MID' | 'LATE' | 'ALL', string> = {
      EARLY: `All ${questCount} early-game quests done. The treasury rewards opening discipline.`,
      MID:   `All ${questCount} mid-game quests done. Real campaign currency for a real campaign run.`,
      LATE:  `All ${questCount} late-game quests done. Apex tier work, apex tier purse.`,
      ALL:   `All ${questCount} quests in the campaign. A legendary clear — capstone bonus paid.`
    };
  ...
  await new Promise<void>(resolve => {
    ...
    // CO-OP LEGION button removed 2026-06-28 (deprioritized). The /coop
    // module + LegionLobby remain in the tree but are no longer reachable
  ...
  (globalThis as any).__oceanEmergenceSfx = () => SFX.oceanMurloc();
