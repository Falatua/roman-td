...
import { comboPreviewBlockHtml } from './render/ComboPreview';
...
async function boot() {
  ...
  function updateProspectSidebar() {
    ...
    if (combos.length > 0) {
      ...
      // bare recipe lines didn't tell them WHAT the combo actually did,
      // forcing a Codex jump. Click the chevron to expand the preview
      // body underneath; click again to collapse. The preview HTML is
      // generated by the shared `comboPreviewBlockHtml` helper so the
      // tower-menu RECIPES list uses the exact same layout.
