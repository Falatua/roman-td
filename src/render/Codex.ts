    ...
    return `
      ...
      ${foldSection('GLOSSARY — KNOW THE WORDS, WIN THE WARS', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px;color:#cdb98a">
          ...
          ${noteCard('DoT', 'Damage Over Time — burn, poison, bleed, and hellfire. Active DoT halves enemy regeneration. All DoTs together are capped at 4% max HP per second on one enemy, with hellfire capped at 1% per second.')}
    ...
    return `
      ...
      ${foldSection('ENEMY SIGNATURES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('🛡 Elephant Dust Shield', '<b style="color:#cdb98a">War Elephants and Undead War Elephants project a 4-tile dust dome</b> that makes nearby ground allies untargetable by ranged towers until the elephant dies. A rotating brown dome surrounds the elephant and protected allies sparkle gold. The elephant remains targetable and melee towers ignore the dust. Siege is the best heavy-hide counter, though living elephants take only +25% siege damage and undead elephants only +5%.')}
      ...
      ${foldSection('BOSS SIGNATURES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('⚔ Hannibal Barca (W10)', 'While his listed War Elephant escorts live, Hannibal heals 0.32%/s after 1.0s without direct damage; his separate out-of-combat regen is 1.34%/s. DoT halves both. At 55% HP, a one-second red telegraph announces his rebirth: 65% HP, cleared statuses, and +60% speed for 10 seconds.')}
      ...
      ${foldSection('STATUS GLOSSARY', `
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;font-size:10px">
          ...
          ${noteCard('STUN', 'Briefly halts an enemy. <b style="color:#ff7766">Bosses are immune to stun</b> so their signature abilities remain active; freeze affects them for half duration. Stunned enemies take <b style="color:#88ddff">+10% direct damage</b>, while DoT damage is unchanged.')}
          ...
          ${noteCard('AGGREGATE DOT CAP (4% max HP/sec)', 'Burn, poison, bleed, and hellfire together can deal at most <b style="color:#88ff88">4% of one enemy\'s max HP per second</b>. Sources add together until the cap; extra DoT beyond it adds no damage. Hellfire has its own 1% per-second limit inside that total. Bosses also reduce DoT damage to 18% of its normal value.')}
          ...
          ${noteCard('DoT items: Mercator and boss rewards', 'DoT items come from Mercator visits and boss legendary rewards, not ordinary drops or the gate shop. <b style="color:#88ddff">Poisoned Blade</b> is MELEE ONLY and applies 6% max HP poison per second for 4 seconds. <b style="color:#ff7733">Fire Oil Flask</b> is RANGED ONLY and applies 4% burn per second for 3 seconds with 1-tile splash. Barbed Gladius adds light melee bleed; Falcata Blade and Alpha Pack Fang provide heavier bleed for any tower.')}
        </div>
      `)}
      ...
      ${foldSection('CROSS-COMBOS (combos-of-combos)', `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('Aurora Legion (T4, 60g)', 'Eagle Standard T4 + Praetorian Wall T4. Deals 116.5 base DPS, pierces 4 enemies in a line, and gains +50% damage against elites. PROVIDENCE BLAST: every 4th attack fires a divine nova in 1.8 tiles around the primary target for 2.5× damage.')}
  ...
  if (tab === 'ITEMS') {
    ...
    return `${section('ITEM EQUIP RULES', '<div style="font-size:11px;color:#cdb98a">Same item stacking is blocked. Family-restricted items are mutually exclusive per tower: a tower cannot carry two speed items, two damage items, two range items, etc. <b style="color:#88ff88">SPECIAL is also capped at one per tower</b>, so trophy/proc effects become strategic placements instead of a single-tower stack. Every item is permanent.<br/><br/><b style="color:#ff9933">Attack-class gates:</b> every restricted item opens its effect with <b style="color:#88ddff">MELEE ONLY</b> or <b style="color:#ff7733">RANGED ONLY</b> in CAPS. The inventory grid greys out incompatible items at equip time.<br/><br/><b style="color:#ff9933">Legendary uniqueness:</b> you can only hold ONE of each legendary at a time. Mercator and boss drops automatically rotate to a different legendary you don\'t already own.</div>')}
      ${section('🟣 EPIC TIER & 🌀 EVENT-EXCLUSIVE LEGENDARIES', `<div style="font-size:11px;color:#cdb98a;line-height:1.55">
        <b style="color:#a060ff">EPIC (purple) tier:</b> 429g base value; shop shelves usually show the vendor-discounted price. <b style="color:#a060ff">Epic items are premium pickups</b> — Gate Shop carries a small randomized Epic shelf, and Mercator carries 2 random Epic slots per visit.
