...
function renderTab(tab: string): string {
  if (tab === 'SYSTEMS') {
    return `
      ${foldSection('NEW HERE? READ THIS BEFORE ROME FALLS', `
        <div style="font-size:11.5px;color:#cdb98a;line-height:1.65;background:#0c0a08;padding:10px 14px;border-left:3px solid #d4af37">
          ...
          <ul style="margin:4px 0 0 14px;padding:0">
            ...
            <li>This is a 30-wave run. Every leaked enemy hurts. Every BOSS leak costs <b style="color:#ff5050">10 lives</b> AND the boss is REBORN ON THE NEXT WAVE at the HP he had when he reached Rome (chip damage carries over — small consolation). The math is still not on your side.</li>
            <li><b style="color:#ffd34d">THE LATE GAME (W21-30, v2):</b> Endless mode was <b>removed</b> — clearing <b>W30</b> with the gate intact is now the win. The campaign extends through a brutal new gauntlet: <b>W21</b> Khan boss, <b>W22-24</b> Mongol + Egyptian push (W24 Anubis King), then <b>W25-29</b> the ROMAN-MYTH gauntlet (Chimera, Cerberus, Typhon, Giants, Cyclopes — tough vs steel/fire but <b style="color:#fff4a8">weak to DIVINE</b>), before the <b style="color:#ff5050">W30 Daemon Imperator</b> finale. From W21 a second cave (CAVE B) erupts open on the left and pours a share of ground enemies down a second lane that merges into the maze.</li>
          </ul>
        </div>
      `, true)}
      ...
      ${foldSection('🌀 SURPRISE EVENTS — INVASION, UPRISING, GATES OF HELL', `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
          ${noteCard('⚔ INVASION (W7, W18)', '<b style="color:#ff7733">The empire is besieged on every side at once.</b> A perimeter fire lights up on the outer edge of the map for <b style="color:#ff7733">EVERY single enemy in the wave</b> — 59 fires on W7, 59 on W18 — and the ground enemies ALL emerge simultaneously the moment the fires finish rising (~0.3s after the event triggers). <span style="color:#88ddff">Flyers don\'t redirect — they keep spawning from the cave normally; only the ground enemies actually emerge from the perimeter.</span> Each invader runs at <b style="color:#ff7733">+25% base speed</b> straight toward the gate. <b style="color:#ffd34d">Skips the first 2 checkpoints</b> — invaders enter the path at WP3, so any towers / aura tiles you parked on WP1 or WP2 are completely bypassed. No spawn cadence, no easing — one unified wall materializing around the entire perimeter at once. <br/><br/><b style="color:#ff9933">Reward pool (Legendary, pick one of 3):</b><br/>• <b>Vanguard Pilum</b> — any tower +35% damage, +1 range<br/>• <b>Aquila Rampart</b> — +50% damage vs enemies above 70% HP (execute role)<br/>• <b>Perimeter Torch</b> — +25% damage AND +25% attack speed')}
          ${noteCard('☠ DEATH UPRISING (W11, W14)', '<b style="color:#a050ff">The dead rise.</b> A single mass-grave urn pulls itself out of the EXACT center tile of the map. Skeletons <b>pour out of the urn</b> in <b style="color:#a050ff">bursts of four</b>, then walk straight to the nearest path tile to join the maze — four risers emerge together in a tight ~0.4-second pulse, then a ~1-second pause before the next burst, repeating until the queue drains. <b style="color:#ffd34d">Skips the first 2 checkpoints</b> — risers enter the path past WP2, so any pressure you parked on WP1/WP2 doesn\'t apply. Overlay around the urn: ground cracks, pulsing dark aura, orbiting floating skulls, a vertical soul column rising from the mouth, ritual blood stains + drifting purple smoke haze. <b style="color:#a050ff">Necromancy is in effect</b> on these waves — killed grunts reanimate as bone variants. <br/><br/><b style="color:#a050ff">Reward pool (Legendary, pick one of 3):</b><br/>• <b>Gravekeeper\'s Scythe</b> — +60% damage vs undead-faction enemies<br/>• <b>Soulfire Brand</b> — applies HELLFIRE on hit (true damage, bypasses every resist)<br/>• <b>Necromancer\'s Lantern</b> — aura 3 tiles: enemies take +25% damage from all sources AND can\'t regen')}
          ${noteCard('🔥 GATES OF HELL (W16)', '<b style="color:#ff4422">The underworld breaks open in four places.</b> <b>FOUR</b> destructible <b>Hell Gate</b> structures rise — <b>two flanking WP3</b> (one west, one east of the path) and <b>two flanking WP4</b> in the same arrangement. Each gate has <b>300,000 HP</b> and contributes to a round-robin <b>Fire Giant</b> spawn schedule (300,000 HP each, slow but tanky): one giant emerges every ~1.5 seconds from a different gate, cycling WP4-east → WP3-east → WP4-west → WP3-west, then repeating. About <b>13 giants</b> total over the 20-second window. <b style="color:#ffd34d">Giants skip the first 2 checkpoints</b> — they spawn AT WP3/WP4 and walk forward, so towers placed earlier on the path never see them. The gates AUTO-SEAL at the 20-second mark, but the player can <b style="color:#ff7733">destroy them early</b> to stop the giant flood — taking out a gate cuts its contribution to the rotation. Fire giants are immune to fire and bleed, vulnerable to divine and siege.<br/><br/><b style="color:#a060ff">Every Fire Giant kill drops a guaranteed EPIC item.</b> ~13 giants over the 20-second window means a serious item haul on top of the legendary surprise-event reward at the end.<br/><br/><b style="color:#ff4422">Reward pool (Legendary, pick one of 3):</b><br/>• <b>Hellgate Brand</b> — +50% damage, +25% attack speed, silence-immune<br/>• <b>Demonsworn Crown</b> — +100% damage vs demons AND +50% vs bosses<br/>• <b>Inferno Standard</b> — aura: +25% damage, applies BURN on hit')}
        </div>
      ...
      ${foldSection('ECONOMY & VENDORS — WHERE THE GOLD GOES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ${noteCard('Gate Shop', 'Refreshes every 4 waves with a freshly randomized lineup (4 Common + 2 Uncommon items, no Rare and no duplicates within a visit, 6 offers total). Always available between waves via the <b>SHOP</b> button, the <b>B</b>/<b>G</b> hotkey, or clicking the gate tile. Lives also purchasable here (5g/+1 life, capped per visit). <b style="color:#ffe066">Barbed Gladius</b> (MELEE-only bleed) is <b>NOT</b> a gate-shop item; it is Mercator-only. You already start every run holding one, so a basic melee-bleed option is reachable from W1 regardless.')}
          ${noteCard('Mercator (W4 / 9 / 14 / 19 / 23 / 27)', 'Traveling merchant. Stocks 4 LEGENDARY trophies (flat 75g each) + 1 Rare + 1 <b style="color:#ff9933">MERCATOR-EXCLUSIVE Rare</b> (Scipio\'s Playbook / Aquilifer\'s Banner / Punic Ledger — never sold at the gate) + 3 mid items (one of which is the exclusive Tyrian Dye). <b style="color:#ffd34d">Traveling Armory</b> now leads with the <b style="color:#f0c040">6 CHAMPIONS OF ROME</b> (350g each — recruit all six and combine them into <b>MARS VICTOR</b>, the divine apex) plus 3 random <b style="color:#ff5050">TIER 5</b> towers at <b style="color:#f0c040">50g each</b>. He gets his own gold-bordered <b>★ MERCATOR</b> button in the HUD (desktop hotkey: <span class="desktop-hotkey-hint"><b>M</b></span>) so the gate shop stays usable beside him — both vendors are open the round he visits. He leaves when you press START WAVE.')}
          ${noteCard('☠ Consumable Traps (gate + Mercator)', 'Buy one-shot traps from <b>either vendor</b>, stockpile them, then SELECT one and click an empty tile near the path to lay it. Each fires once on contact and is spent. They are drawn bright + pulsing so they are easy to spot, do NOT block the path, and pop a damage number on every hit. Six types: <b style="color:#c0c0c0">Iron Spike</b> (heavy hit), <b style="color:#66dd44">Venom</b> (hit + POISON), <b style="color:#ff7722">Tar Fire</b> (hit + BURN), <b style="color:#88ddff">Frost Snare</b> (wide SLOW), the BOSS-specialist <b style="color:#ffcc44">Ballista Snare</b> (4x vs bosses), and the FLYER-specialist <b style="color:#cfe0ff">Sky Net</b> (the only trap fliers can trip).')}
          ${noteCard('🎰 Fortuna\'s Wheel — Combo Tower Gamble', 'Mercator-only mechanic. Pay <b style="color:#f0c040">500g</b> for a <b>RANDOM COMBO TOWER</b> drawn from a pool of 33. Most combos T2 through T5 are on the wheel, but the <b>5 apex super-combos</b> (Imperium Eternum, Carthage Scourge, Triumvirate, Legion Prime, Consular Fatebinder) are EXCLUDED and must be crafted, not bought. <b style="color:#ff5050">Tier rarity is linear:</b> per-spin tier odds are <b style="color:#b87333">T2 40%</b> · <b style="color:#c0c0c0">T3 30%</b> · <b style="color:#ffd34d">T4 20%</b> · <b style="color:#ff5050">T5 10%</b> (weighted 4/3/2/1, then uniform within tier). A specific T5 averages ~230 spins (~115k gold); landing ANY T5 averages ~10 spins. <b style="color:#ff5050">Pure RNG, no pity, no per-visit cap.</b> The spin animation cycles through random portraits before landing on the winner; the tower joins your placement queue exactly like a Mercator T5 buy. Spin counter on the section header tracks how many times you\'ve tempted the goddess this visit.')}
      ...
      ${foldSection('📊 SCORE — HOW THE LEADERBOARD MEASURES YOU', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('Lives + Gold (end of run)', 'At end-of-run: <b style="color:#88ff88">+100 per surviving life</b> and <b style="color:#f0c040">+10 per unspent gold</b>. Cushion = points. Hoarding pays in two ways.')}
          ...
          ${noteCard('Victory Bonus', 'Clearing W30 awards a flat <b style="color:#ffd34d">+10,000</b> score. Survival is the main score lever; victory is the crown.')}
    ...
    return `
      ${section('QUESTS — RUN-LONG OBJECTIVES', `
        ...
        <div style="background:#1a1410;border:2px solid #d4af37;padding:10px 12px;margin:8px 0;font-size:11.5px;color:#cdb98a;line-height:1.55">
          ...
          • All six <span style="color:#88ddff;font-weight:bold">EARLY</span> quests → <b style="color:#88ff88">+50g</b><br/>
          • All six <span style="color:#ffd34d;font-weight:bold">MID</span> quests → <b style="color:#88ff88">+100g</b><br/>
          • All six <span style="color:#ff5050;font-weight:bold">LATE</span> quests → <b style="color:#88ff88">+200g</b><br/>
          • <b style="color:#ffd34d">Clear all 18</b> → an additional <b style="color:#88ff88">+500g grand-completion capstone</b> on top of the LATE bonus.
        </div>
    ...
    return `
      ...
      ${foldSection('⚠ SURPRISE EVENTS — scheduled chaos', `
        <div style="margin-bottom:8px;padding:10px 12px;background:#2a0c08;border:2px solid #ff7733;color:#ffd1cc;font-size:12px;line-height:1.5">
          <b style="color:#ff7733">Fixed campaign schedule.</b> Invasion on W7 and W18, Uprising on W11 and W14, Gates of Hell on W16. Endless mode rolls one on ~75% of waves (2-wave cooldown). Each event pays a 3-card LEGENDARY reward on clear.
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ${noteCard('⚔ Invasion (W7, W18)', 'Perimeter fires light up around the entire map — one per enemy in the wave queue (~59 fires on both W7 and W18). Each invader walks in from its own edge tile toward the path, converging on the middle. Cave spawn is disabled for the wave; the maze still works because invaders snap onto the path past the 3rd waypoint and must still touch the back half. Screen tints warm red, low-brass sting plays.')}
          ${noteCard('☠ Skeletal Uprising (W11, W14)', 'A single skull urn rises from the dead center tile of the map. Undead pour from that one point in bursts of four. Screen washes purple, dissonant organ stab plays. Like Invasions, risers snap to the path past the 3rd waypoint, so the back-half maze still pressures them. Only triggers on undead waves.')}
          ${noteCard('🔥 Gates of Hell (W16)', 'FOUR destructible Hell Gates (300,000 HP each) rise — two flanking WP3, two flanking WP4 — and round-robin Fire Giants every ~1.5 seconds for 20 seconds (~13 giants total). Destroy gates early to cut their contribution to the rotation. <b style="color:#a060ff">Every Fire Giant kill drops a guaranteed EPIC item.</b>')}
          ...
          ${noteCard('🛡 Portals are cosmetic', 'Fires and urns can\'t be destroyed — they light up, deliver their enemies, and fade. The exception is Gates of Hell, where the gates themselves ARE targetable (300,000 HP). Focus fire on the spawned enemies otherwise.')}
      ...
      ${foldSection('ENEMY SIGNATURES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('🛡 Elephant Dust Shield', '<b style="color:#cdb98a">War Elephants and Undead War Elephants project a 4-tile dust dome</b> that makes every NEARBY GROUND enemy untargetable by ranged towers until the elephant dies. Visual: dust-brown rotating dome around the elephant + small gold sparkle over each protected ally. The elephant itself stays ranged-targetable (you can still chip it down). <b style="color:#88ddff">Melee towers ignore the dust</b> — they hit allies inside the dome normally. Best counter: <b style="color:#ff9933">siege towers</b> (elephants take +45% from siege).')}
      ...
      ${foldSection('⚔ v2 LATE-GAME GAUNTLET (W21-30) — NEW UNITS', `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('🏛 Mars Victor (Siege+Divine apex — fuses all 6 heroes)', 'Recruit the <b>6 CHAMPIONS OF ROME</b> at Mercator (350g each: Marius, Agrippa, Agricola, Scipio, Caesar, Sulla) and combine all six → <b>MARS VICTOR</b>. <b style="color:#ffd34d">792 DPS</b> (more than the six heroes combined), range 8, crit 28% × 3.2, wide-splash Wrath that lands as <b style="color:#fff4a8">BOTH Siege and Divine</b> (resolves against whichever the target resists less). <b style="color:#fff4a8">FUSES EVERY HERO PASSIVE into one army-wide aura while it stands:</b> +25% damage AND +15% attack speed to EVERY tower, <b>ALL towers may strike flyers</b> (Agricola), and +25% damage vs bosses (Scipio). Never gambled or bought, only forged from the six Champions. The single biggest power spike in the game.')}
        </div>
      `)}
  ...
  if (tab === 'ENEMIES') {
    ...
    const surpriseSummary = `<div data-codex-row style="background:#0c0a08;border:1px solid #3a3025;padding:10px 12px;margin-bottom:10px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
        ${noteCard('⚔ INVASION · W7 + W18', 'A perimeter fire lights up for every enemy in the wave queue (about 59 fires on both W7 and W18) along every edge. The wave\'s enemies spawn from the fires (not the cave) at <b style="color:#ff7733">+25% speed</b>, all emerging at once. <b style="color:#ffd34d">Skips the first 2 checkpoints</b> — every invader joins the path at WP3.')}
        ${noteCard('☠ DEATH UPRISING · W11 + W14', 'A single skull urn rises from the EXACT center tile of the map. Enemies <b>pour out of the urn</b> in bursts of four, then walk to the nearest path tile to join the maze. <b style="color:#a050ff">Necromancy active</b>: killed grunts reanimate as bone variants. Floating skulls + soul columns overlay the urn. <b style="color:#ffd34d">Skips the first 2 checkpoints</b> — risers enter past WP2.')}
        ${noteCard('🔥 GATES OF HELL · W16', 'FOUR destructible <b style="color:#ff4422">Hell Gates</b> rise — two flanking WP3, two flanking WP4 (each pair separated by 2 tiles, 300,000 HP each). They round-robin <b style="color:#ff4422">Fire Giants</b> every ~1.5s for 20s (~13 giants total). Destroy the gates early to cut their contribution to the rotation. <b style="color:#ffd34d">Giants spawn AT WP3/WP4</b> — the first 2 checkpoints are bypassed entirely. <b style="color:#a060ff">Every Fire Giant drops a guaranteed EPIC item on kill.</b>')}
      </div>
    </div>`;
  ...
  if (tab === 'ITEMS') {
    ...
    for (const rarity of RARITY_ORDER) {
      ...
      inTier.sort((a, b) => ((a[1] as any).buy ?? 0) - ((b[1] as any).buy ?? 0));
    ...
    return `${section('ITEM EQUIP RULES', '<div style="font-size:11px;color:#cdb98a">Same item stacking is blocked. Family-restricted items are mutually exclusive per tower: a tower cannot carry two speed items, two damage items, two range items, etc. <b style="color:#88ff88">Exception: DoT items (burn / poison / bleed) and SPECIAL trophies stack freely.</b> Every item is permanent.<br/><br/><b style="color:#ff9933">Attack-class gates:</b> every restricted item opens its effect with <b style="color:#88ddff">MELEE ONLY</b> or <b style="color:#ff7733">RANGED ONLY</b> in CAPS. The inventory grid greys out incompatible items at equip time.<br/><br/><b style="color:#ff9933">Legendary uniqueness:</b> you can only hold ONE of each legendary at a time. Mercator and boss drops automatically rotate to a different legendary you don\'t already own.</div>')}
      ${section('🟣 EPIC TIER & 🌀 EVENT-EXCLUSIVE LEGENDARIES', `<div style="font-size:11px;color:#cdb98a;line-height:1.55">
        <b style="color:#a060ff">EPIC (purple) tier:</b> 60g buy / 30g sell. <b style="color:#a060ff">Mercator-exclusive</b> — gate shop never stocks them; Mercator carries 2 random Epic slots per visit. Items: Berserker's Muzzle, Celtic Longsword, Necrotic Longsword, Lictor's Fasces, Auxiliary Sling, Optio's Whistle.
  ...
  if (tab === 'HEROES') {
    ...
    `);
    return `${section('THE ROSTER', '<div style="font-size:11px;color:#cdb98a;line-height:1.5">Six historical Roman generals stand in the hero pool. Every run shows <b style="color:#fff8e0">all six in a horizontal-scroll picker</b> — only one champion serves you per campaign. Each hero unlocks <b style="color:#fff8e0">2 abilities</b> (one at LEGATUS, one at CONSUL) and carries up to <b style="color:#fff8e0">6 equipped items</b> — gear is the primary expression lever. Heroes earn XP from every kill on the field (+1 non-boss / +20 boss) and tier up through five ranks; the basic-attack scale still climbs 1.0× → 2.4× across all five tiers, with the IMPERATOR and DIVUS milestones being stat-only steps. Heroes occupy a single tile and cannot be sold, combined, moved, or downgraded.</div>')}
      ${forgeSection}
      ${cards}`;
  }
  if (tab === 'WAVES') {
    const rows = (waves as any[]).map(w => {
      ...
      return `<tr><td style="color:#d4af37;font-weight:bold">${w.wave}</td><td>${flag}${necroTag}${reliefTag}${healDisabledTag}</td><td style="color:#9be0ff">${w.faction}</td><td>${w.gold}g</td><td style="opacity:0.85">${spawns}</td></tr>`;
    }).join('');
...
const BOSS_SCRIPTS_FOR_CODEX: Record<string, string[]> = {
  ...
  UNDEAD_WAR_ELEPHANT: [
    ...
    'DUST-SHIELD AURA — protects nearby ground allies from ranged attacks while alive',
    'WEAKNESS: takes +20% damage from SIEGE (V31 trim — undead hide is denser than living elephant hide; living variant still +45%)'
  ],
  HANNIBAL_BARCA: [
  ...
  DAEMON_IMPERATOR: [
    ...
    'W30 FINAL BOSS — any leak ends the run instantly (Rome falls)'
  ]
};
