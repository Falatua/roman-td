...
function renderTab(tab: string): string {
    ...
    return `
      ${foldSection('NEW HERE? READ THIS BEFORE ROME FALLS', `
        <div style="font-size:11.5px;color:#cdb98a;line-height:1.65;background:#0c0a08;padding:10px 14px;border-left:3px solid #d4af37">
          ...
          <br/><br/>
          <b style="color:#ffd34d">The twist that breaks new players:</b> you don't pick which tower you place. Every round rolls <b>5 random prospects</b> (each empty-tile click costs <b>1g</b> to place one on the field). Then you KEEP up to <b>two</b>. Everything else turns to stone. The maze you accidentally build is the maze your enemies must walk.
          <br/><br/>
      ...
      ${foldSection('GLOSSARY — KNOW THE WORDS, WIN THE WARS', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px;color:#cdb98a">
          ...
          ${noteCard('DoT', 'Damage Over Time — burn, poison, bleed, hellfire. Ticks while you sleep. Also reduces enemy regen by 50% (was 100% pre-2026-05-21). Aggregate DoT is capped at 8% maxHP/sec per enemy (hellfire 2%/sec sub-cap).')}
      ...
      ${foldSection('PROGRESSION — HOW YOU GROW (OR DO NOT)', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('Per-Tower Crit Chance', 'Every tower has its own crit profile. Fast attackers (Pugio Assassin 25%, Eques signature 25%) crit often for modest bonuses. Heavy siege (Colossus Onager 6%, Vulcan Engineer 6%) crits rarely for huge multipliers (×3.0–×3.5).')}
      ...
      ${foldSection('ECONOMY & VENDORS — WHERE THE GOLD GOES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('Boss-Kill Gold Bonus', 'Every boss kill pays <b>22 + round(wave × 3.5) gold</b> on top of wave-end gold AND on top of the baseline +1g. W20 final boss ≈ +92g + 1g. Stack with Aerarium kills for spikes.')}
    ...
    return `
      ...
      ${foldSection('⚠ SURPRISE EVENTS — scheduled chaos', `
        <div style="margin-bottom:8px;padding:10px 12px;background:#2a0c08;border:2px solid #ff7733;color:#ffd1cc;font-size:12px;line-height:1.5">
          <b style="color:#ff7733">Fixed campaign schedule.</b> Invasion on W7 and W18, Uprising on W11 and W14, Gates of Hell on W16. Endless mode rolls one ~75% of waves (2-wave cooldown). Each event pays a 3-card LEGENDARY reward on clear.
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ${noteCard('⚔ Invasion (W7, W18)', 'Perimeter fires light up around the entire map — one per enemy in the wave queue. Every invader walks in from their own edge tile toward the path, converging on the middle. Cave spawn is disabled for the wave; the maze still works because invaders snap onto the path past the 3rd waypoint and must still touch the back half. Screen tints warm red, low-brass sting plays.')}
      ...
      ${foldSection('WAVE STRUCTURE', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ${noteCard('Boss Crescendo (W5/10/15/20)', 'A boss anchors every 5th wave. <b>Basic-enemy HP doubles</b> each cleared boss; <b>bosses themselves scale linearly</b> so they\'re a real step but not a wall. <b>Tower damage gains +50% per cleared boss</b> to partly keep pace (×1.50 per step vs basics\' ×2.00 — the gap is the design pressure). W20 is the climactic Daemon Imperator solo fight.')}
          ...
          ${noteCard('Twin Bosses (W15 only, 25%)', 'On <b>W15</b> only, the scheduled boss wave has a 25% chance to spawn a SECOND boss alongside the authored one. Kill him: <b style="color:#88ff88">2.5× wave gold +50g flat, +2 lives restored, +1 free RARE item, +3000 score</b>. Let him leak: 10 lives AND he returns next wave at the HP he leaked at (chip damage carries). Banner: 🎲 TWIN BOSSES. <b style="color:#ff5050">W20 is RNG-free</b> — no twin bosses there.')}
          ${noteCard('Ambush Bosses (W17-W19, 15%)', 'Any non-boss wave from W17 to W19 has a 15% chance to ambush you with an extra boss. Same payout: <b style="color:#88ff88">2.5× wave gold +50g +2 lives +1 RARE item +3000 score</b> if killed, 10 lives per leak otherwise. Banner: 🎲 AMBUSH BOSS. A free UNCOMMON item drops on ARRIVAL as a consolation, whether you finish him or not. <b style="color:#ff5050">W20 is excluded</b> — the Daemon Imperator gets the stage to himself.')}
          ${noteCard('Wave Modifier (30% from W3)', 'A random mutation (Blood Moon / Storm Surge / Death Pact / Veil / Revenant / Group March) lands on the wave. The gods drop a <b>free Uncommon item</b> on arrival, and surviving pays an extra <b style="color:#88ff88">+60g RNG bonus + 4000 score</b>. The modifier rolls AFTER you press START — you cannot prepare specifically, only build resilient. The TOP-RIGHT chip pulses while an RNG wave is active; tap or hover the <b>"RNG WAVE"</b> tag for the exact mechanic + survival reward.')}
          ...
          ${noteCard('20-Wave HP Curve', 'BASIC enemies: HP climbs <b>+10% per wave linearly</b>, plus an extra <b>+10% per wave from W11</b>, plus another <b>+15% per wave from W12</b>, all multiplied by <b>2.00× per cleared 5-wave boss</b>. BOSSES: linear-only progression — they ignore the per-5-wave doubling and only ride the linear stack. Each boss feels like a real step heavier than the last, not a brick wall.')}
        </div>
      `)}
      ...
      ${foldSection('STATUS GLOSSARY', `
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;font-size:10px">
          ...
          ${noteCard('AGGREGATE DOT CAP (8% maxHp/sec)', 'Total per-tick damage from all DoTs (burn + poison + bleed + hellfire) on a single enemy is clamped to <b style="color:#88ff88">8% maxHp/sec</b>. Below the cap, sources stack additively; above the cap, additional sources have no extra effect. <b style="color:#ffaa66">HELLFIRE carves out a tighter 2%/sec sub-cap</b> inside the aggregate (it\'s permanent + bypasses resists, so it earns the tighter leash). Applies universally from W1. Boss DoT reduction (×0.18) already shrinks per-source damage on bosses, so the cap mostly bites on regular waves where DoT stacking used to hit 14–15%/sec.')}
      ...
      ${foldSection('BOSS WAVE — NEW RULES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('No Atmospheric VFX', 'Boss waves stay visually clean — no surprise events, no wave modifiers, no twin/ambush boss rolls on W20. The play area is reserved for the boss fight so you can focus on positioning.')}
  ...
  if (tab === 'ENEMIES') {
    ...
    const surpriseSummary = `<div data-codex-row style="background:#0c0a08;border:1px solid #3a3025;padding:10px 12px;margin-bottom:10px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
        ${noteCard('⚔ INVASION · W7 + W18', '18 perimeter fires light up along every edge. The wave\'s enemies spawn from the fires (not the cave) at <b style="color:#ff7733">+25% speed</b>. Tight 0.18s spawn cadence — the breach pours. <b style="color:#ffd34d">Skips the first 2 checkpoints</b> — every invader joins the path at WP3.')}
