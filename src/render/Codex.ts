    ...
    return `
      ...
      ${foldSection('⚠ SURPRISE EVENTS — mid-wave ambushes', `
        <div style="margin-bottom:8px;padding:10px 12px;background:#2a0c08;border:2px solid #ff7733;color:#ffd1cc;font-size:12px;line-height:1.5">
          <b style="color:#ff7733">🎲 NOT TELEGRAPHED IN ADVANCE.</b> Roughly <b>4 events per 20-wave campaign</b> (2 Invasions + 2 Uprisings on average). The exact waves are intentionally left off the wave-preview chip — you find out the moment the audio sting + screen tint + camera shake fire. Endless mode rolls a 25% chance per wave instead.
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ${noteCard('⚔ Invasion', '<b style="color:#ff7733">FOUR FIRE BREACHES</b> erupt around the map perimeter (one per edge). The ENTIRE wave\'s spawn queue routes through them round-robin, so enemies pour in from every direction at once (cave spawn disabled for this wave). Each enemy snaps onto the nearest path tile <b>at or past Waypoint 3</b> — invaders skip WP1 + WP2 (they didn\'t walk from the cave) but still have to touch WP3 → 4 → 5 → 6 → 7 in order before reaching the gate, so your back-half maze pressure still applies. The screen tints warm red and a low-brass sting plays.')}
          ${noteCard('☠ Skeletal Uprising', 'Only fires during <b style="color:#aa66ff">undead waves</b>. A <b style="color:#aa66ff">SINGLE SKULL URN</b> erupts from the ground on the EXACT center tile of the map with a ground-crack particle and a discordant organ sting. The wave\'s spawn queue routes through that one urn — undead pour from the dead center in bursts of four. Like the Invasion, risen undead snap to the path <b>at or past Waypoint 3</b> — they skip WP1 + WP2 (they didn\'t walk from the cave) but still need to touch WP3 → 4 → 5 → 6 → 7 in order. The screen washes sickly purple. Enemies overlap your towers harmlessly (the urn just rises through them; the towers stay).')}
          ...
          ${noteCard('🎁 Reward — your pick', 'After the last surprise enemy dies (or leaks), the game pauses and a 3-card choice modal opens. Pick ONE item from a <b style="color:#7ec3ff">RARE</b> / <b style="color:#7ee07e">UNCOMMON</b> roll — no DoT items (those stay Mercator-exclusive). If your inventory is full at pick time, the reward is forfeit, so clear a slot the moment the sting fires.')}
          ${noteCard('⏱ Rules', 'Minimum <b>3 waves</b> between any two events. Boss waves (W5 / W10 / W15 / W20) and flyer waves never roll one. Uprisings additionally require an undead-faction wave (W11+). The events are scheduled at fixed waves under the hood — the codex won\'t spoil which ones, but the wave timing is consistent run-to-run.')}
      ...
      ${foldSection('ENEMY SIGNATURES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('Stealth Cycle', 'Ghost Rider (~0.8s every 5s) and Shadow Cavalry (~1.0s every 6s) fade to untargetable on a regular pulse — towers cannot acquire them while stealthed. (W6 Celtic Scout was previously on this list; the stealth flavor was retired so the W6 flyer wave plays as a straight DPS test.)')}
      ...
      ${foldSection('STATUS GLOSSARY', `
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;font-size:10px">
          ...
          ${noteCard('FREEZE', 'Enemy is fully locked in place for the duration — zero movement, and knockback cannot displace them either. Bosses take half-duration freezes.')}
