    ...
    return `
      ...
      ${foldSection('PROGRESSION — HOW YOU GROW (OR DO NOT)', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard(`Quest System (${QUESTS.length} quests + tier bonuses)`, `Run-long objectives with tiered per-quest rewards (gold / items / free towers). Tracked in the bottom-right HUD panel; full list in the QUESTS tab. <b style="color:#88ff88">Clearing every quest in a tier banks an extra gold bonus on top of the individual quest rewards:</b> all ${QUESTS.filter(q => q.tier === 'EARLY').length} EARLY quests = <b>+50g</b>, all ${QUESTS.filter(q => q.tier === 'MID').length} MID = <b>+100g</b>, all ${QUESTS.filter(q => q.tier === 'LATE').length} LATE = <b>+200g</b>, and clearing every quest in the campaign pays a <b style="color:#ffd34d">+500g grand-completion capstone</b>. Killing bosses ALSO pays a flat bonus of <b>22 + round(wave × 3.5) gold</b>.`)}
        </div>
      `)}
