    ...
    return `
      ...
      ${foldSection('⚠ SURPRISE EVENTS — scheduled chaos', `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('🔥 Gates of Hell (W16)', 'Two destructible Hell Gates (100,000 HP each) rise at the middle of the path and pump out Fire Giants every 2 seconds for 15 seconds, alternating gates. Destroy the gates early to stop the flood. <b style="color:#a060ff">Every Fire Giant kill drops a guaranteed EPIC item.</b>')}
          ...
          ${noteCard('🛡 Portals are cosmetic', 'Fires and urns can\'t be destroyed — they light up, deliver their enemies, and fade. The exception is Gates of Hell, where the gates themselves ARE targetable (100,000 HP). Focus fire on the spawned enemies otherwise.')}
      ...
      ${foldSection('ENEMY SIGNATURES', `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          ...
          ${noteCard('🥷 Ambush Stealth (NEW 2026-05-21)', '<b style="color:#a078d0">Carthage Spearman (W7-W10) and Undead Berserker (W13/W15)</b> emerge from cover during the opening seconds of their wave. For the <b>first 15 seconds</b> of each wave, any alive instance of these enemies is UNTARGETABLE — towers visually see the silhouette at 40% opacity but cannot acquire. When the 15-second window expires, EVERY alive instance becomes targetable simultaneously (a coordinated "the line emerges" beat). Spawns AFTER the 15s window are normally targetable from spawn. <b style="color:#88ff88">Counters:</b> melee blockers + cleave towers can soak the emergence wave; AURA towers (Triarius +12%, Caesar +10%) make the post-emergence kill window sharper.')}
  ...
  if (tab === 'ENEMIES') {
    ...
    const surpriseSummary = `<div data-codex-row style="background:#0c0a08;border:1px solid #3a3025;padding:10px 12px;margin-bottom:10px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
        ...
        ${noteCard('🔥 GATES OF HELL · W16', 'Two destructible <b style="color:#ff4422">Hell Gate</b> structures (100,000 HP each) rise at WP3 and WP4. Each pumps out a <b style="color:#ff4422">Fire Giant</b> (100,000 HP, slow + bulky) every 2s for 15s, alternating gates. Destroy the gates early to stop the flood. <b style="color:#a060ff">Every Fire Giant drops a guaranteed EPIC item on kill.</b>')}
      </div>
    </div>`;
...
function renderEnemyCard(id: string, def: any, ctx: any, allWaves: number[]): string {
  ...
  if (def.ambushStealth) traits.push(`AMBUSH STEALTH — untargetable for the first ${def.ambushStealthSec ?? 15}s of each wave (visual: alpha 40%). When the window expires, every alive instance becomes targetable at once. Spawns AFTER the window are visible from the start.`);
  // Tower disruption
